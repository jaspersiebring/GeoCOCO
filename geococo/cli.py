...
def build_coco(image_path: pathlib.Path, labels_path: pathlib.Path, json_path: pathlib.Path, output_dir: pathlib.Path, image_size: Tuple[int, int], category_attribute: str = "category_id") -> None:
    """
    This method generates a COCO dataset by moving across the given image (image_path) with 
    a moving window (image_size), constantly checking for intersecting annotations (labels_path)
    that represent image objects in said image (e.g. buildings in satellite imagery; denoted by
    category_attribute). Each valid intersection will add n Annotations entries to the dataset 
    ...
    :param image_size: Size of the output images in format 'width height'
    """
    ...
    elif not category_attribute in labels.columns:
