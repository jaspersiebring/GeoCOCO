...
def test_add_categories_by_ids():
    ...
    assert np.array_equal(
        np.array([cat.id for cat in dataset.categories]), np.unique(category_ids)
    )
    assert np.array_equal(
        np.array([cat.name for cat in dataset.categories]),
        np.unique(category_ids).astype(str),
    )
...
def test_add_categories_by_names():
    ...
    assert np.array_equal(
        np.array([cat.id for cat in dataset.categories]),
        np.arange(1, np.unique(category_names).size + 1),
    )
    assert np.array_equal(
        np.array([cat.name for cat in dataset.categories]), np.unique(category_names)
    )
...
def test_add_categories_by_names_and_ids():
    ...
    assert np.all(
        np.isin(
            np.unique(category_ids), np.array([cat.id for cat in dataset.categories])
        )
    )
    assert np.all(
        np.isin(
            np.unique(category_names),
            np.array([cat.name for cat in dataset.categories]),
        )
    )
...
def test_add_categories_with_supers():
    ...
    assert len(dataset.categories) == np.unique(category_ids).size
    assert np.all(
        np.isin(np.unique(category_ids), [cat.id for cat in dataset.categories])
    )
    assert np.all(
        np.isin(np.unique(category_names), [cat.name for cat in dataset.categories])
    )
    assert np.all(
        np.isin(
            np.unique(supercategory_names),
            [cat.supercategory for cat in dataset.categories],
        )
    )
